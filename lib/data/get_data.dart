...
class DataFromAPI {
  ...
  static Future<List<RealTimeEachStationArrival>> getStationList(List<Pair<Pair<String, String>, int>> addedStation) async {
    ...
    for (Pair<Pair<String, String>, int> cur in addedStation) {
        ...
        for (int i = 0; i < fetchData["realtimeArrivalList"].length; i++) {
          ...
          if (target["subwayId"] == line) {
            if (target["updnLine"] == "상행") {
              rst.last.up.add(Pair(parseAheadString(target["bstatnNm"]), parseString(target["arvlMsg2"])));
            } else if (target["updnLine"] == "하행") {
              rst.last.down.add(Pair(parseAheadString(target["bstatnNm"]), parseString(target["arvlMsg2"])));
            } else {
              print("fail");
            }
          }
      ...
      int index = 0;
      for (StationInform iter in SubwayListDataSet.allStationData) {
        if (iter.kName == rst.last.kName && iter.line == rst.last.line) {
          break;
        }
        index++;
      }
      if (index - 1 >= 0 && SubwayListDataSet.allStationData[index - 1].line == rst.last.line) {
        rst.last.nextName = SubwayListDataSet.allStationData[index - 1].kName;
      }
      if (index + 1 < SubwayListDataSet.allStationData.length && SubwayListDataSet.allStationData[index + 1].line == rst.last.line) {
        rst.last.prevName = SubwayListDataSet.allStationData[index + 1].kName;
      }
  ...
  static Future<void> initStationData() async {
    SubwayListDataSet.allStationData = await mapData2List(await getAllStationName());
  }

  static Future<SubwayListDataSet> getSubwayInfo() async {
    // Boss!!
    HiveProvider mainHive = HiveProvider();
    var SubData = SubwayListDataSet();
  ...
  static Future<Map<String, Map<String, List<String>>>> getAllStationName() async {
